...
const config = new ConfigurationManager<AppConfig>().addConfig({
...
// 启动应用
app.start();
console.log("app", app, AccountApi, KernelApi, AnyStoreApi);
...
let store =
  app.services.resolve<Store<AuthorizationStore>>("AuthorizationStore");

export { app, accountApi, kernelApi, storage, store, anyStoreApi };
